...
public class gameCode : MonoBehaviour {
    ...
    public bool wentToEndGame = false;
    //prevents game from trying to loadLevel thousands of times
    ...
    public float turnStart = 3;
    //^^ when should the next turn start (set to 5 secs after game starts, to give player a bit to orient)
    ...
    public int streakBonusPoints = 10;
    public int bonusPointMultiplier;
    public int totalCrosses;
    public int bonusPointsEarned;
    //stats
    public static int totalBlackPunishmentCubes = 0;
    public static int totalRainbowsAwarded = 0;
    public static int totalSameColorsAwarded = 0;
    public static int totalBonusesEarned = 0;
    ...
    public void RainbowCrossChecker()
    {
        ...
        for (int rowToCheck = 1; rowToCheck < numRowsofCubes - 1; rowToCheck++)
        {
            ...
            {
                //only run if the cube has a color (isn't white/black/grey)
                if (cubeGrid[colToCheck, rowToCheck].GetComponent<cubeBehavior>().hasAColor == true)
                {
                    //only run if the surrounding cubes have color
                    if (cubeGrid[colToCheck + 1, rowToCheck].GetComponent<cubeBehavior>().hasAColor == true &&
                        cubeGrid[colToCheck - 1, rowToCheck].GetComponent<cubeBehavior>().hasAColor == true &&
                        cubeGrid[colToCheck, rowToCheck + 1].GetComponent<cubeBehavior>().hasAColor == true &&
                        cubeGrid[colToCheck, rowToCheck - 1].GetComponent<cubeBehavior>().hasAColor == true)
                    {
                            ...
                            cubeGrid[colToCheck, rowToCheck].GetComponent<cubeBehavior>().yellowTouch)
                        {
                            ...
                            cubeGrid[colToCheck, rowToCheck].GetComponent<cubeBehavior>().whatColor = "grey";
                            cubeGrid[colToCheck + 1, rowToCheck].GetComponent<cubeBehavior>().whatColor = "grey";
                            cubeGrid[colToCheck - 1, rowToCheck].GetComponent<cubeBehavior>().whatColor = "grey";
                            cubeGrid[colToCheck, rowToCheck + 1].GetComponent<cubeBehavior>().whatColor = "grey";
                            cubeGrid[colToCheck, rowToCheck - 1].GetComponent<cubeBehavior>().whatColor = "grey";
    ...
    public void SameColorCrossChecker()
    {
        ...
        for (int rowToCheck = 1; rowToCheck < numRowsofCubes - 1; rowToCheck++)
        {
            for (int colToCheck = 1; colToCheck < numColsofCubes - 1; colToCheck++)
            {
                //only run if the cube is not white/black/grey
                if (cubeGrid[colToCheck, rowToCheck].GetComponent<cubeBehavior>().hasAColor == true)
                {
                    //only run if the surrounding cubes are also not white/black/grey
                    if (cubeGrid[colToCheck + 1, rowToCheck].GetComponent<cubeBehavior>().hasAColor == true &&
                        cubeGrid[colToCheck - 1, rowToCheck].GetComponent<cubeBehavior>().hasAColor == true &&
                        cubeGrid[colToCheck, rowToCheck + 1].GetComponent<cubeBehavior>().hasAColor == true &&
                        cubeGrid[colToCheck, rowToCheck - 1].GetComponent<cubeBehavior>().hasAColor == true)
    ...
    //determines how many bonus points to give (if any)
    //for every 2 crosses created in 1 turn, player gets an extra 10 points
    public void streakCounter()
    {
        totalCrosses = rainbowsToAward + sameColorsToAward;
        while (totalCrosses > 1)
        {
            totalCrosses = totalCrosses - 2;
            bonusPointMultiplier++;
            totalBonusesEarned++;
        }
        bonusPointsEarned = streakBonusPoints * bonusPointMultiplier;
    }

    //adds points to player score
    public void AwardPoints()
    {
        if (rainbowsToAward + sameColorsToAward >= 2)
        {
            streakCounter();
            playerPoints += bonusPointsEarned;
        }
        if (rainbowsToAward > 0)
        {
            totalRainbowsAwarded += rainbowsToAward;
        ...
        if (sameColorsToAward > 0)
        {
            totalSameColorsAwarded += sameColorsToAward;
    ...
    //watches for a valid key to be pressed
    ...
    public void PlayerClickedCubeGrid(GameObject clickedCube)
    {
		...
		else if (clickedCube.GetComponent<cubeBehavior>().hasAColor == true)
		{
			//Player clicked on a cube that isn't white/black/grey
	...
	void Start () {
        ...
        for (int rowOfCubes = 0; rowOfCubes < numRowsofCubes; rowOfCubes++)
        {
            for (int CubesInRow = 0; CubesInRow < numColsofCubes; CubesInRow++)
            {
                cubeGrid[CubesInRow, rowOfCubes] = (GameObject)Instantiate(baseCube, new Vector3(CubesInRow * 2 - 10f, rowOfCubes * 2 - 4f, 10f), Quaternion.identity) as GameObject;
        ...
        if (gameOn == true)
        {
            ...
            if (TimeForNewCubeColor())
            {
                ...
                else
                {
                    ...
                    TurnACubeBlack();
                    totalBlackPunishmentCubes++;
                    //they also lose one point (not to go below zero)
                    if (playerPoints > 0)
                    {
                        playerPoints--;
                    }
            ...
            scoreUI.text = "Points: " + playerPoints;
        }
        else if (wentToEndGame == false)
        {
            goToEndGame();
        }
	}
